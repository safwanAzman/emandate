...
class Bypasslogin extends Controller
{
    public function loggingin2(Request $request)
    {
        ...
        $decrypt = DB::select(DB::raw("select uf_decrypt_string('$appkey') as decryptVal from dual"));
		//$aa = $decrypt;
		//dd($aa);
		//die;
        ...
        $user = FMS_USERS::where('USERID',strtoupper($fmsuserid))->first();
		//$user = FMS_USERS::where('USERID',$fmsuserid)->first();
        //dd($user);
        //die; 
        if($user != null)
        {
			$userid = ($fmsuserid);
			//dd($userid);
			$savedpassword = ($fmspwd);  //$this->decrypting
			//dd($savedpassword);
            if($savedpassword === $fmspwd) {
				//dd('pass');
				//die;
